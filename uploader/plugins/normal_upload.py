...
from .ytdl_upload import generate_screen_shots, generate_sample, complete_process
...
async def default(c, m):
    ...
    if not sts:
        return await m.message.edit('The link you provided is invalid, not accessible or not direct download link.\n\n**So try to load it on [transloader](http://t.me/transload), then send me a new link**', disable_web_page_preview=True)
    ...
    try:
        ...
        await m.message.edit("Sorry i am very busy now")
    else:
        ...
        try:
            ...
            if not os.path.exists(download_directory):
                if id in Config.ACTIVE_DOWNLOADS:
                    try:
                        await m.message.edit("**Download Failed!!**", parse_mode="markdown")
                    ...
                    try:
                        await trace.edit(text=f"**Name:** {m.from_user.mention(style='md')}\n\nID:** {m.from_user.id}\n\n**UserName:** @{m.from_user.username}\n\n**Link:** {m.message.reply_to_message.text}\n\n**Status:** Process Cancelled by user", parse_mode='markdown')
                ...
                try:
                    if screenshots != 0:
                        asyncio.get_event_loop().create_task(generate_screen_shots(c, m, download_directory, tmp_directory_for_each_user, screenshots, duration))
                    if samplevideo != 0:
                        asyncio.get_event_loop().create_task(generate_sample(download_directory, c, m.message.reply_to_message))
                except:
                    pass

                try:
                    await m.message.edit(text=f"**FileName:** `{caption_name}`\n\n__Preparing to upload__")
                    await trace.edit(text=f"**Name:** {m.from_user.mention(style='md')}\n\n**id:** {m.from_user.id}\n\n**UserName:** @{m.from_user.username}\n\n**Link:** {m.message.reply_to_message.text}\n\n**Status:** Uploading 📤", parse_mode='markdown')
                except:
                    pass
                ...
                if not as_file:
                    try:
                        ...
                        try:
                            ...
                            await trace.edit(text=f"**Name:** {m.from_user.mention(style='md')}\n\n**ID:** {m.from_user.id}\n\n**UserName:** @{m.from_user.username}\n\n**Link:** {m.message.reply_to_message.text}\n\n**Status:** FloodWait {e}", parse_mode='markdown')
                        except:
                            pass
                        del Config.TIME_GAP1[m.from_user.id]
                        continue 
                        ...
                        try:
                            await m.message.edit(f"--Error:--\n{e}")
                            await trace.edit(text=f"**Name:** {m.from_user.mention(style='md')}\n\n**ID:** {m.from_user.id}\n\n**UserName:** @{m.from_user.username}\n\n**Link:** {m.message.reply_to_message.text}\n\n**Status:** Error while uploading {e}", parse_mode='markdown')
                        except:
                            pass
                        del Config.TIME_GAP1[m.from_user.id]
                        continue 
                ...
                if (final_media is None)|(id not in Config.ACTIVE_DOWNLOADS):
                    try:
                        if not id in Config.ACTIVE_DOWNLOADS:
                            ...
                            await trace.edit(text=f"**Name:** {m.from_user.mention(style='md')}\n\n**id:** {m.from_user.id}\n\n**UserName:** @{m.from_user.username}\n\n**Link:** {m.message.reply_to_message.text}\n\n**Status:** Cancelled By user", parse_mode='markdown')
                        elif time.time() - start_time > 1200:
                            await m.message.edit("Process Cancelled Due to timeout of 20min.")
                            await trace.edit(text=f"**Name:** {m.from_user.mention(style='md')}\n\n**id:** {m.from_user.id}\n\n**UserName:** @{m.from_user.username}\n\n**Link:** {m.message.reply_to_message.text}\n\n**Status:** Cancelled due to TimeOut", parse_mode='markdown')
                        else:
                            ...
                            await trace.edit(text=f"**Name:** {m.from_user.mention(style='md')}\n\n**id:** {m.from_user.id}\n\n**UserName:** @{m.from_user.username}\n\n**Link:** {m.message.reply_to_message.text}\n\n**Status:** upload Failed", parse_mode='markdown')
                    except:
                        pass
                    try:
                        del Config.TIME_GAP1[m.from_user.id]
                ...
                try:
                    await trace.edit(text=f"**Name:** {m.from_user.mention(style='md')}\n\n**id:** {m.from_user.id}\n\n**UserName:** @{m.from_user.username}\n\n**Link:** {m.message.reply_to_message.text}\n\n**Status:** Uploaded Sucessfully ✅", parse_mode='markdown')
...
async def download_coroutine(c, m, session, url, download_directory, start, id, trace):
    downloaded = 0
    display_message = ""
    last_edit = 0
    async with session.get(url, timeout=Config.PROCESS_MAX_TIMEOUT) as response:
        try:
            ...
            await m.message.edit("**Download Failed**\n\nUnable to extract Size. Please try again later. If again fails try different link.", parse_mode="markdown")
            return False
        ...
        if "text" in content_type and total_length < 500:
            ...
            return True 
        try:
            await m.message.edit(text=f"__**Initiating Download**__\n\n**🔗 URL:** {url}\n\n**📁 File Name:** {os.path.basename(download_directory)}\n\n**💽 File Size:** {humanbytes(total_length)}")
        except:
            pass
        with open(download_directory, "wb") as f_handle:
            while True:
                ...
                if id in Config.ACTIVE_DOWNLOADS:
                    if now - start < 1800:
                        if now + 5 >= last_edit or downloaded == total_length:
                            last_edit = now
                            ...
                            try:
                                current_message = """__**Downloading:** {}%__
[{}{}]
    ...
    TimeFormatter(time_to_completion)
                                ...
                                try:
                                    await m.message.edit(
                                        ...
                                        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("CANCEL 🚫", callback_data=f"cancel_download+{id}")]])
                                    )
                                except:
                                     pass
                            except Exception as e:
                        ...
                        try:
                            await trace.edit(text=f"**Name:** {m.from_user.mention(style='md')}\n\n**ID:** {m.from_user.id}\n\n**UserName:** @{m.from_user.username}\n\n**Link:** {m.message.reply_to_message.text}\n\n**Status:** Process Cancelled(timeout)", parse_mode='markdown')
                            await m.message.edit("Process Cancelled\n\n**Reason: Timeout Error**", parse_mode="markdown")
                        except:
                            pass
