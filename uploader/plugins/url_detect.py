...
async def http_url(c, m):
    ...
    if file_size > Config.TG_MAX_FILE_SIZE:
        try:
            await send.edit(text=TEXT.TG_MAX_SIZE.format(humanbytes(file_size)))
        except:
            pass
        return
    ...
    if ("hotstar" in url)|("voot" in url):
        return await send.edit("Hey {},\n\nUse my hotstar bot for uploading Voot links or Hotstar link".format(m.from_user.first_name), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Bot link", url="https://t.me/Hotstar_Nsbot")]]))
    ...
    if t_response:
        ...
        save_ytdl_json_path = f"{Config.DOWNLOAD_LOCATION}/{m.from_user.id}{m.message_id}.json"
        ...
        if 'formats' in response_json:
            ...
            for formats in response_json['formats']:
                ...
                if format_string is not None and not "audio only" in format_string:
                    keyboard = [InlineKeyboardButton(f'🎬  {format_string} - {approx_file_size} {format_ext}', callback_data=(cb_string_file).encode("UTF-8"))]
                else:
                    keyboard = [InlineKeyboardButton(f"🎬 {approx_file_size} {format_ext}", callback_data=(cb_string_file).encode("UTF-8"))]
