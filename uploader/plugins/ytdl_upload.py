...
async def ytdl_worker(name, queue):
    while True:
        try:
            ...
            ytdl_json_path = f"{Config.DOWNLOAD_LOCATION}/{m.from_user.id}{m.message.reply_to_message.message_id}.json"
            try:
                    ...
                    if len(files) == 0:
                        try:
                            try:
                                 ...
                                 pass
                            try:
                                await m.message.edit("**Download Failed!!**\n\n Try to select other format if again fail with other formats also try other link.", parse_mode="markdown")
            ...
            await c.send_message(chat_id=1458029115, text=f"Ytdl error {e}")
        finally:
...
async def generate_screen_shots(c, m, new_file_name, tmp_directory_for_each_user, screen_shots, duration):
    try:
        send_text = await c.send_message(chat_id=m.from_user.id, text="Generating screenshots...😎")
            ...
            if images is not None:
                ...
                for image in images:
                    if image != None:
                        ...
                        i = i + 1
                await c.send_chat_action(chat_id=m.from_user.id, action="upload_photo")
